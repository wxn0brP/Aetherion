...
const hex = (max = 0xfffffff) =>
	"0x" + Math.floor(Math.random() * max).toString(16);
...
log.push(
	30,
	`vm_page_bootstrap: ${Math.floor(os.freemem() / 4096)} free pages and ${Math.floor(os.totalmem() / 4096 - os.freemem() / 4096)} wired pages`,
);
log.push(
	30,
	`kext submap [${hex()} - ${hex()}], kernel text [${hex()} - ${hex()}]`,
);
...
cpuList.forEach((_, idx) => {
	...
	log.push(
		10,
		`AetherionACPICPU: ProcessorId=${idx} LocalApicId=${idx} ${status}`,
	);
});
